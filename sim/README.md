...
Simulation will exit upon receiving `SIGINT` (Ctrl+C) or `SIGTERM` (kill). The simulator will enter debug mode upon encountering an illegal opcode (unofficial control wordd `0xFFFE`) or a microcode fault (unofficial control word `0xFFFF`).
