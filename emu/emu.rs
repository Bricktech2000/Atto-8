  ...
  fn tick(
    ...
    match instruction {
      ...
      Instruction::Rot(size) => {
        ...
        mem_write!(addr, res);
        mp.cf = false;
        Ok((18 + size.get() as u128) * (top as u128 + 1))
      }
      ...
      Instruction::Ldo(ofst) => {
        let addr = mp.sp.wrapping_add(ofst.get());
        sp_push!(mem_read!(addr));
        Ok(12 + ofst.get() as u128)
      }
