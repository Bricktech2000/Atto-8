...
main!
  ...
  loop:
    ...
    # wait for controller buttons to be released
    x02 !stall_long

    # wait until a controller button is pressed
    x00 wait: pop !getc !char.null xor :wait !bcs
    x00 # 0x00 as default
      !secondary_up xo2 x01 iff x10 xo2
      !secondary_down xo2 xFF iff x20 xo2
    ad2
    x00 # 0x00 as default
      !primary_up xo2 xF0 iff x01 xo2
      !primary_down xo2 x10 iff x02 xo2
      !primary_left xo2 xFF iff x04 xo2
      !primary_right xo2 x01 iff x08 xo2
    st0 ad2
  :loop !jmp
...
draw_circle! # draw_circle(u8 r, u4u4 cycy)
  ...
  nop !u4u4
  # t1 = r / 16
  x02 !u8 # magic constant 0x02 seems to draw better circles
  while.
