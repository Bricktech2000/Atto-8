...
// major parts of this file are identical to '/mic/mic.rs'

fn main() {
  let args: Vec<String> = std::env::args().collect();
  if args.len() != 2 {
    eprintln!("BF-Mic: Usage: bf-mic <microcode image file>");
    std::process::exit(1);
  }
                ...
                (wrapped, true) => {
                  if step == 0x00 {
                    errors.extend([Error(format!(
                      "Microcode for opcode {:02X} with carry {:01b} overflows by {} steps",
                      opcode,
                      carry as u8,
